...
#   - Total download size is about 200-400 MB (test_set subset only).
#   - First run takes about 5-15 min depending on bandwidth.

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
DEST="${1:-$ROOT/data/dns2020}"
...
if [ "$NOISY_COUNT" -lt 100 ] || [ "$CLEAN_COUNT" -lt 100 ]; then
    echo "WARNING: expected at least 150 wavs each. If counts are low, ensure git-lfs pulled blobs." >&2
