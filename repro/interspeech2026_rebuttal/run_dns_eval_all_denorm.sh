#!/usr/bin/env bash
# Same as run_dns_eval_all.sh but writes to results/rebuttal/dns_eval_denorm/
# and passes --dnsmos_scale denorm to src.evaluate_dns. Intrusive metrics
# (PESQ/STOI/CSIG/CBAK/COVL/segSNR) are unchanged across policies; only the
# DNSMOS rows differ. Resume-friendly: skips runs whose JSON already exists.
#
# Inputs (assumed):
...
#
# Output:
#   results/rebuttal/dns_eval_denorm/D_<MODEL>_<SEED>_voicebank.{json,log}
#   results/rebuttal/dns_eval_denorm/C_<MODEL>_<SEED>_dns2020.{json,log}
...
OUT="$ROOT/results/rebuttal/dns_eval_denorm"
mkdir -p "$OUT"
...
run_one() {
    ...
    echo "[$kind] $model/$seed -> RUN ($ckpt) [denorm]"
